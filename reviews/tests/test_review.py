...
class ReviewCreateTest(APITestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.review_data = {'content': '모성의 목소리 조차 박탈한 뒤 그 크신 사랑만을 돌림노래로 부르는 공업적 최루법.', 'rating': '2.0'}
...
class ReviewReadTest(APITestCase):
    ...
    def setUpTestData(cls):
        ...
        for i in range(10):
            ...
            cls.reviews.append(Review.objects.create(content=cls.faker.sentence(), rating=4.5, movie=cls.movie, user=cls.user))
...
class ReviewDetailTest(APITestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.movie.genres.set(Genre.objects.filter(name__in=cls.movie_data['genres']))
        cls.review_data = {'content': '모성의 목소리 조차 박탈한 뒤 그 크신 사랑만을 돌림노래로 부르는 공업적 최루법.', 'rating': '2.0'}
        cls.review_data_new = {'content':'전편보다 낫다는 이야기를 듣고 보았다. 그렇긴 했다.', 'rating':'2.5'}
        cls.user = User.objects.create_user('leedddd', 'test@testuser.com', 'Qwerasdf1234!')
...
class ReviewLikeTest(APITestCase):
    ...
    def setUpTestData(cls):
        ...
        # Movie와 Genre가 ManyToMany관계라서 .set() 메소드로 설정합니다.
        cls.movie.genres.set(Genre.objects.filter(name__in=cls.movie_data['genres']))
        cls.review_data = {'id': 1,'content': '모성의 목소리 조차 박탈한 뒤 그 크신 사랑만을 돌림노래로 부르는 공업적 최루법.', 'rating': '2.0'}
        cls.user = User.objects.create_user('leedddd', 'test@testuser.com', 'Qwerasdf1234!')
    ...
    def test_like_review(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data, 'like 했습니다')
    ...
    def test_unlike_review(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data, 'unlike 했습니다')